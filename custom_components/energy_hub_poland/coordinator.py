...
class PGEDataCoordinator(DataUpdateCoordinator):
    ...
    async def _fetch_with_retries(self, fetch_date: date) -> Optional[Dict[int, float]]:
        api_query_date = fetch_date - timedelta(days=1)

        for attempt in range(10):
            _LOGGER.debug(
                "Pobieranie danych dla %s (data zapytania API: %s), próba %d",
                fetch_date, api_query_date, attempt + 1
            )
            ...
            _LOGGER.warning(
                ...
                fetch_date, attempt + 1
            )
            await asyncio.sleep(30)

        _LOGGER.error("Nie udało się pobrać danych dla %s po wielu próbach.", fetch_date)
        return None
    ...
    async def _async_update_data(self) -> Dict[str, Any]:
        ...
        if self._internal_data["today_date"] and self._internal_data["today_date"] < today_date:
            _LOGGER.debug(
        ...
        if not self._internal_data["today"] or self._internal_data["today_date"] != today_date:
            _LOGGER.info("Brak danych na dziś (%s). Rozpoczynam pobieranie.", today_date)
        ...
        if not self._internal_data["tomorrow"] or self._internal_data["tomorrow_date"] != tomorrow_date:
            _LOGGER.info(
                "Brak danych na jutro (%s). Rozpoczynam pobieranie.",
                tomorrow_date
            )
    ...
    async def _load_cache(self):
        try:
            ...
            if cached:
                ...
                self._internal_data = {
                    "today": {int(k): v for k, v in cached.get("today", {}).items()} if cached.get("today") else None,
                    "today_date": date.fromisoformat(cached["today_date"]) if cached.get("today_date") else None,
                    "tomorrow": {int(k): v for k, v in cached.get("tomorrow", {}).items()} if cached.get("tomorrow") else None,
                    "tomorrow_date": date.fromisoformat(cached["tomorrow_date"]) if cached.get("tomorrow_date") else None,
                }
    ...
    async def _save_cache(self):
        try:
            data_to_save = {
                "today": self._internal_data["today"],
                "today_date": self._internal_data["today_date"].isoformat() if self._internal_data["today_date"] else None,
                "tomorrow": self._internal_data["tomorrow"],
                "tomorrow_date": self._internal_data["tomorrow_date"].isoformat() if self._internal_data["tomorrow_date"] else None,
            }
